...
item <- sample(unique(dat$item_id), 1)
pdf("analyse_proportion_correct/Proportion_correct.pdf", width = 7, height = 3.5)
...
for (i in item) {
  ...
  text(-4.4,.9, paste("a =",round(fit$coefficients["delta"],3)), pos=4)
  text(-4.4,.7, paste("n =",length(data$delta)), pos=4)
}

dev.off()

# Copy plot to manuscript
system("cp analyse_proportion_correct/Proportion_correct.pdf manuscript/images/")
